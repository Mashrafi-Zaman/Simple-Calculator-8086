# Simple Calculator in 8086 Assembly 
...
1. **Reads user input** via DOS interrupt (`int 21h`):
   - First operand (a digit `0`–`9`)
   - Arithmetic operator (`+`, `-`, `*`, `/`)
   - Second operand (a digit `0`–`9`)

2. **Converts ASCII characters** to numeric values by subtracting `'0'`.

3. **Performs the specified arithmetic operation**:
   - Addition
   - Subtraction
   - Multiplication
...
5. **Formats and displays the result**:
